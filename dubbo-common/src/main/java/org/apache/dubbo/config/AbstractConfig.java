...
public abstract class AbstractConfig implements Serializable {
    ...
    /**
     * vergilyn-comment, 2020-03-13 >>>> <br/>
     * 通过反射获取目标对象的 getter 方法，并调用该方法获取属性值。然后再通过 getter 方法名解析出属性名，比如从方法名 getName 中可解析出属性 name。 <br/>
     * 如果用户传入了属性名前缀，此时需要将属性名加入前缀内容。 <br/>
     * 最后将 <属性名，属性值> 键值对存入到 map 中就行了。 <br/>
     * @param parameters
     * @param config
     * @param prefix
     */
